...
void MyEventAction::EndOfEventAction(const G4Event *event)
{
    ...
    if(dose > 0.)
    {
        fRunAction->AddDdep(dose);  //functia dependenta de dose numita AddDdep()
        fRunAction->AddDdep2(dose);  //functia dependenta de dose numita AddDdep2()

        sum += dose;  //doza totala (pt. verificare cu ce afiseaza la final de run)
